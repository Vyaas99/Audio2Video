...
def load_primary_models(pretrained_model_path):
    ...
    return noise_scheduler, tokenizer, text_encoder, vae, unet, CAVP, at_embedder
    ...
    if cached_latent_dir is None:
        ...
        for i, batch in enumerate(tqdm(train_dataloader, desc="Caching Latents.")):
            ...
            try:
                ...
                # audio_values -> features
                # batch['audio_values'] = beats.extract_features(audio_values)[1]
                _, spec1 = get_spectrogram(audio_values.cpu().numpy(), 160000, from_tensor = True)
                spec1 = torch.tensor(spec1).unsqueeze(0).to('cuda')
                batch['audio_values'] = CAVP.encode_spec(spec1, pool=False).float()
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    noise_scheduler, tokenizer, text_encoder, vae, unet, CAVP, at_embedder = load_primary_models(pretrained_model_path)
    ...
    freeze_models([vae, text_encoder, unet, CAVP]) 
    # freeze_models([vae, text_encoder, CAVP])  # also train the generation model
    ...
    models_to_cast = [text_encoder, vae, unet, CAVP]
    ...
    def finetune_unet(batch, train_encoder=False):
        ...
        for i in range(audio_values.shape[0]):
            _, spec1 = get_spectrogram(audio_values[i,:].cpu().numpy(), 160000, from_tensor = True)
            spec1 = torch.tensor(spec1).unsqueeze(0).to('cuda').float()
            features = CAVP.encode_spec(spec1, pool=False)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % checkpointing_steps == 0:
                    ...
                    # add .module if using DDP 
                    torch.save(at_embedder.state_dict(), f"{output_dir}/at_embedder_{global_step}.pth")
                    torch.save(unet.state_dict(),        f"{output_dir}/unet_{global_step}.pth")
...
# CUDA_VISIBLE_DEVICES=1,2,3 accelerate launch train_cavp.py --config  configs/v2/vggsound_subset.yaml --run_name train_cavp