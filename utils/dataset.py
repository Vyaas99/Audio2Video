...
def process_video(vid_path, use_bucketing, w, h, get_frame_buckets, get_frame_batch):
    ...
    waveform = torchaudio.functional.resample(waveform, orig_freq=sample_rate, new_freq=16000) # Added: convert sr to 16000
    sample_rate = 16000
    if waveform.size()[0] == 2:
        waveform = waveform[:1, :] # Convert from stereo to mono by selecting only the first channel

    waveform = torch.tile(waveform, (1, 10))
    audio = waveform[:, :sample_rate * 10]

    start = int(sample_rate * (idxs[0])/30) # what does the /30 and /24 do here?
...
class VideoFolderDataset(Dataset):
    def __init__(
        ...
        use_bucketing: bool = False,
        labels=None,          # Super confusing operation!
        data_set='train',
        repeat=5,
        **kwargs
    ):
    ...
    def get_video_shape(self):
        ...
        return torch.Size([self.n_sample_frames, 3, 384, 512]) # for thunder
    ...
    def __getitem__(self, index):
        ...
        while True:
            try:
                ...
                # print(video[0].size(), self.get_video_shape())
                assert video[0].size() == self.get_video_shape()
                # print(audio.size(), self.get_audio_shape())
                assert audio.size() == self.get_audio_shape() # torch.Size([2, 44100]) torch.Size([1, 16000])
